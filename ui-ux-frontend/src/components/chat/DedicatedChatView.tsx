...
- **Ready to Explore**: You can open this region in the Satellite Viewer to inspect high-resolution imagery.`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 flex flex-col h-[calc(100vh-64px)] select-none relative bg-black text-white">
      {/* Top Header Card */}
      <div className="relative z-10 p-4 rounded-2xl bg-gray-900/90 border border-white/10 shadow-2xl backdrop-blur-md mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-white transition-colors duration-200">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-emerald-950/60 text-emerald-300 text-[11px] font-semibold border border-emerald-500/30 flex items-center gap-1.5 font-mono">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <select
            ...
            className="px-3 py-1.5 rounded-xl bg-black/50 border border-white/10 text-xs font-medium text-gray-200 outline-none focus:border-cyan-500 font-mono"
          >
          ...
          <button
            ...
            className="p-2 rounded-xl text-gray-400 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-500/40 transition-all duration-150 active:scale-[0.98] cursor-pointer"
          ...
          <button
            ...
            className="p-2 rounded-xl text-gray-400 hover:text-rose-400 bg-white/5 hover:bg-rose-950/30 border border-white/10 hover:border-rose-500/40 transition-all duration-150 active:scale-[0.98] cursor-pointer"
            title="Clear Chat"
          >
      ...
      <div className="relative z-10 flex-1 rounded-2xl bg-gray-900/90 border border-white/10 shadow-2xl backdrop-blur-md flex flex-col overflow-hidden text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 bg-black/40">
          ...
          {messages.length === 1 && (
            <div className="p-5 rounded-2xl bg-white/5 border border-white/10 flex flex-col sm:flex-row items-center gap-5 shadow-xs backdrop-blur-md">
              <OrbitMascot size="lg" mood="greeting" className="flex-shrink-0" />
              <div className="space-y-1.5 text-center sm:text-left">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-950/60 text-cyan-300 text-xs font-semibold border border-cyan-500/30">
          ...
          {messages.map((msg) => (
            ...
            >
              {/* Sender Header */}
              <div className="flex items-center gap-2 text-xs text-gray-400 mb-1 px-1 font-mono">
                {msg.sender === 'user' ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="text-cyan-400 font-semibold">Orbit AI</span>
                  </>
                )}
                <span>•</span>
              ...
              <div
                className={`p-4 sm:p-5 rounded-2xl max-w-[90%] sm:max-w-[80%] leading-relaxed ${
                  msg.sender === 'user'
                    ...
                    : 'bg-gray-900 border border-white/10 text-gray-200 rounded-tl-sm shadow-md'
                }`}
              >
                ...
                {msg.traces && msg.traces.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-white/10">
                    <button
                      ...
                      className="w-full flex items-center justify-between text-xs font-semibold text-cyan-400 hover:text-cyan-300 active:scale-[0.98] py-1 cursor-pointer"
                    >
                    ...
                    {expandedTraceId === msg.id && (
                      <div className="mt-3 space-y-2 text-xs bg-black/50 p-3.5 rounded-xl border border-white/10 animate-in fade-in">
                        {msg.traces.map((trace) => (
                          <div
                            key={trace.id}
                            className="p-2.5 rounded-lg bg-gray-950 border border-white/10 space-y-1 shadow-xs"
                          >
                            ...
                            {trace.payload && (
                              <div className="pl-5 pt-1">
                                ...
                                {showRawJsonId === trace.id && (
                                  <pre className="mt-1 p-2 rounded bg-black border border-white/10 text-cyan-300 text-[10px] overflow-x-auto font-mono">
                ...
                {msg.groundedStats && (
                  <div className="mt-4 pt-3 border-t border-white/10 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                    {msg.groundedStats.map((stat, i) => (
                      <div
                        key={i}
                        className="p-2.5 rounded-xl bg-black/50 border border-white/10 font-mono"
                      >
                ...
                {msg.suggestedAction && (
                  <div className="mt-4 pt-3 border-t border-white/10 flex items-center gap-3">
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-blue-600 text-white text-xs font-semibold flex items-center gap-1.5 shadow-xs hover:bg-blue-500 transition-all duration-150 active:scale-[0.98] cursor-pointer"
                    >
          ...
          {isProcessing && (
            <div className="flex flex-col items-start animate-in fade-in">
              <div className="p-4 rounded-2xl bg-gray-900 border border-white/10 text-white rounded-tl-sm flex items-center gap-3.5 text-xs shadow-md">
        ...
        <div className="p-3 border-t border-white/10 bg-gray-950 overflow-x-auto flex items-center gap-2">
          <span className="text-[11px] font-semibold uppercase text-gray-400 flex items-center gap-1 flex-shrink-0 font-mono">
            <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
            Quick Prompts:
          </span>
          {samplePrompts.map((p, idx) => (
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-500/40 text-xs font-medium text-gray-300 hover:text-white whitespace-nowrap transition-all duration-150 active:scale-[0.98] shadow-xs flex items-center gap-1 cursor-pointer"
            >
              <span>{p.title}</span>
        ...
        <div className="p-4 border-t border-white/10 bg-gray-950">
          {attachedFile && (
            <div className="mb-2 p-2 rounded-lg bg-blue-950/60 border border-cyan-500/30 text-xs text-cyan-300 flex items-center justify-between font-mono">
          ...
          >
            ...
            <button
              ...
              className="p-2.5 rounded-xl border border-white/10 bg-white/5 text-gray-400 hover:text-white hover:border-cyan-500/40 hover:bg-white/10 transition-all duration-150 active:scale-[0.98] cursor-pointer"
            ...
            <input
              ...
              disabled={isProcessing}
              className="flex-1 px-4 py-2.5 rounded-xl bg-black/50 border border-white/10 text-white placeholder-gray-500 outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 text-xs sm:text-sm"
            />

            <button
              type="submit"
              disabled={!inputQuery.trim() || isProcessing}
              className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center gap-1.5 shadow-xs transition-all duration-150 active:scale-[0.98] focus:outline-none disabled:opacity-40 cursor-pointer"
            >
