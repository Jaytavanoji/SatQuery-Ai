  ...
  return (
    <>
      ...
      >
        ...
        {isOpen && (
          <div
            className={`bg-gray-950/95 border border-white/10 shadow-2xl flex flex-col overflow-hidden text-white transition-all duration-300 ease-out pointer-events-auto backdrop-blur-xl ${
              isExpanded
          ...
          >
            {/* Top Radiant Accent Trim */}
            <div className="h-0.5 w-full bg-gradient-to-r from-cyan-500 via-blue-600 to-emerald-500 shrink-0"></div>

            {/* Header */}
            <div className="px-3 py-2.5 bg-gray-900/80 border-b border-white/10 text-white flex items-center justify-between shadow-xs shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <div className="text-xs font-bold text-white flex items-center gap-1.5">
                    ...
                    {isExpanded && (
                      <span className="ml-1 text-[9px] font-semibold text-cyan-300 bg-cyan-950/60 px-1.5 py-0.5 rounded border border-cyan-500/30">
                        Full
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-1">
                ...
                {hasStartedChatting && !isExpanded && (
                  <button
                    ...
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold text-cyan-300 bg-cyan-950/40 hover:bg-cyan-900/50 border border-cyan-500/30 transition-all duration-150 active:scale-[0.98]"
                ...
                {isExpanded && (
                  <button
                    onClick={() => setIsExpanded(false)}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold text-gray-300 bg-white/5 hover:bg-white/10 border border-white/10 transition-all duration-150 active:scale-[0.98]"
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-150 active:scale-[0.98]"
                  title="Close Window"
                >
                  <X className="w-4 h-4" />
                </button>
            ...
            {/* Messages Area */}
            <div className="flex-1 p-3 overflow-y-auto space-y-2.5 text-xs bg-black/60 custom-scrollbar">
              {messages.map((m) => (
                <div
                  key={m.id}
                ...
                >
                  ...
                  <div
                    className={`p-2.5 rounded-2xl ${isExpanded ? 'max-w-[82%]' : 'max-w-[88%]'} text-[11.5px] leading-relaxed ${
                      m.sender === 'user'
                        ? 'bg-gradient-to-r from-cyan-600 to-blue-600 text-white font-medium rounded-tr-sm shadow-md'
                        : 'bg-gray-900/90 border border-white/10 text-gray-200 rounded-tl-sm shadow-md backdrop-blur-md'
                    }`}
                  >
                    ...
                    {m.suggestedAction && (
                      <div className="mt-2 pt-2 border-t border-white/10">
                        <button
                          ...
                          className="px-2 py-1 rounded-lg bg-cyan-950/40 hover:bg-cyan-900/50 border border-cyan-500/30 text-cyan-300 text-[10px] font-semibold flex items-center gap-1 active:scale-[0.98] transition-colors"
                        >
              ...
              {isProcessing && (
                <div className="flex items-center gap-2 text-[11px] text-gray-400 px-2 py-1">
              ...
              {hasStartedChatting && !isExpanded && !isProcessing && (
                <div className="pt-1.5 pb-0.5">
                  <div className="p-2 rounded-xl bg-cyan-950/30 border border-cyan-500/20 flex items-center justify-between gap-2 shadow-xs">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <MessageSquare className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                      <span className="text-[10.5px] text-gray-300 font-medium truncate">
                        Need more space?
                      </span>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsExpanded(true)}
                      className="px-2 py-0.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-semibold text-[10px] shrink-0 flex items-center gap-1 active:scale-[0.98] transition-colors shadow-xs"
                    >
            ...
            {/* Quick Starter Chips */}
            <div className="py-1.5 px-2 border-t border-white/10 bg-gray-950/80 overflow-x-auto flex gap-1.5 text-[10px] shrink-0">
              {quickQueries.map((q, i) => (
                <button
                  key={i}
                  onClick={() => handleSend(q)}
                  disabled={isProcessing}
                  className="px-2 py-1 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 hover:border-cyan-500/40 text-gray-300 hover:text-cyan-300 whitespace-nowrap active:scale-[0.98] transition-all shadow-xs"
                >
                  {q}
                </button>
              ))}
            ...
            <form
              ...
              className="p-2 px-2.5 border-t border-white/10 bg-gray-900/90 backdrop-blur-md flex items-center gap-1.5 shrink-0"
            >
              <input
                type="text"
                value={inputQuery}
                ...
                disabled={isProcessing}
                className="flex-1 px-2.5 py-1.5 rounded-xl bg-black/60 border border-white/10 text-xs text-white placeholder-gray-500 outline-none focus:border-cyan-500"
              />
              <button
                type="submit"
                disabled={!inputQuery.trim() || isProcessing}
                className="p-1.5 sm:p-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-semibold shadow-xs active:scale-[0.98] disabled:opacity-40 transition-colors"
              >
        ...
        {!isOpen && (
          <button
            ...
            className="group pl-2.5 pr-4 py-2 rounded-full bg-gray-900/90 text-white font-semibold text-xs shadow-xl hover:shadow-cyan-500/10 border border-white/15 flex items-center gap-2.5 transition-all duration-150 active:scale-[0.98] focus:outline-none hover:-translate-y-0.5 hover:border-cyan-400 backdrop-blur-md"
