...
export const CoreFeaturesSection: React.FC<CoreFeaturesSectionProps> = ({ onLaunchWithQuery }) => {
  ...
  return (
    <section id="features" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-black border-b border-white/10">
      {/* Section Header */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {features.map((feature) => {
          ...
          return (
            <div
              key={feature.id}
              className="p-6 rounded-2xl bg-gray-900/90 hover:bg-gray-900 border border-white/10 hover:border-cyan-500/40 shadow-xl backdrop-blur-xl transition-all duration-200 flex flex-col justify-between group"
            >
              <div className="space-y-4">
                {/* Top Badge & Icon */}
                <div className="flex items-center justify-between">
                  <div className="w-11 h-11 rounded-xl flex items-center justify-center bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-900/30">
                ...
                {/* Example Query Pill */}
                <div className="p-2.5 rounded-xl bg-black/50 border border-white/10 text-[11px]">
                  <span className="text-gray-500 block text-[10px] uppercase font-bold">Sample Question:</span>
                  <span className="text-cyan-200 font-semibold italic">
              ...
              <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between text-xs">
                <span className="text-gray-500 text-[11px]">Easy for anyone</span>
                {onLaunchWithQuery && (
                  <button
                    onClick={() => onLaunchWithQuery(feature.exampleQuery, feature.mode)}
                    className="flex items-center gap-1 font-bold text-cyan-400 hover:text-cyan-300 hover:underline active:scale-95 transition-all cursor-pointer"
                  >
