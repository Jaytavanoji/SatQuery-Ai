    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-200 text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 space-y-2.5">
          <span className="text-xs font-mono font-bold uppercase tracking-wider text-gray-400 block mb-1">
          ...
          {MOCK_ERROR_SCENARIOS.map((scenario) => {
            ...
            return (
              <button
                ...
                className={`w-full p-3.5 rounded-xl border text-left transition-all duration-150 flex items-center justify-between cursor-pointer ${
                  isSelected
                    ? 'bg-rose-950/40 border-rose-500/50 text-white shadow-sm ring-1 ring-rose-500/40'
                    : 'bg-white/5 border-white/10 text-gray-300 hover:border-rose-400/40 hover:text-white'
                }`}
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <FileWarning className={`w-4 h-4 flex-shrink-0 ${isSelected ? 'text-rose-400' : 'text-gray-400'}`} />
                  <span className="text-xs font-bold truncate">{scenario.title}</span>
                </div>
                <span className="text-[10px] font-mono text-gray-400 uppercase ml-2 flex-shrink-0">
        ...
        <div className="lg:col-span-8 p-6 sm:p-8 rounded-2xl bg-gray-900/90 border-2 border-rose-500/30 shadow-xl space-y-6 backdrop-blur-md">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            ...
            <span className="px-2.5 py-1 rounded-full bg-rose-500/15 text-rose-300 border border-rose-500/30 font-mono text-xs font-bold">
          ...
          <div className="p-4 rounded-xl bg-cyan-950/30 border border-cyan-500/30 space-y-2 text-xs font-mono">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              disabled={isResolved}
              className={`px-5 py-2.5 rounded-xl font-mono text-xs font-bold flex items-center gap-2 transition-all active:scale-95 shadow-md cursor-pointer ${
                isResolved
