    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-200 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {displayList.map((tool) => (
          <div
            key={tool.id}
            className="p-6 rounded-2xl bg-gray-900/90 border border-white/10 hover:border-cyan-500/40 hover:shadow-xl transition-all duration-200 flex flex-col justify-between group backdrop-blur-md"
          >
            <div className="space-y-4">
              {/* Header: Title, Badge, Status */}
              <div className="flex items-center justify-between">
                <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-950/60 text-cyan-300 border border-cyan-500/30">
                  {tool.badge}
                </span>
              ...
              <div className="space-y-2 pt-2 border-t border-white/10 text-xs">
                <div>
                  <span className="text-[10px] text-gray-400 uppercase font-bold block font-mono">Supported Input:</span>
                  <span className="text-gray-200 font-semibold">{tool.supportedInput}</span>
                </div>

                <div className="grid grid-cols-2 gap-2 pt-1 text-[11px] font-mono">
                  <div>
                    <span className="text-[10px] text-gray-400 uppercase block">Architecture:</span>
                    <span className="text-gray-300">{tool.architecture.slice(0, 22)}...</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-gray-400 uppercase block">Accuracy:</span>
                    <span className="text-gray-300">{tool.precision}</span>
                  </div>
                </div>

                <div className="p-2.5 rounded-lg bg-black/40 border border-white/10 text-[11px] text-cyan-300 font-bold flex items-center justify-between font-mono">
            ...
            {onSelectModelTask && (
              <div className="mt-5 pt-3 border-t border-white/10 flex items-center justify-between text-xs">
                <span className="text-gray-400 font-mono">Response: ~{tool.latencyMs}ms</span>
                <button
                  onClick={() => onSelectModelTask(tool.taskType as AnalysisTaskType)}
                  className="flex items-center gap-1.5 font-bold text-cyan-400 hover:text-cyan-300 active:scale-95 transition-all cursor-pointer font-mono"
                >
