...
import { EvidenceViewer } from './VisualEvidenceSplitter';
import { ExecutionSummary } from './AnalysisSummaryView';
import { 
  ImageAnalysisMode, 
  UploadedImageMeta, 
  AnalysisResultData, 
            ...
            >
              {parts.map((part, pIdx) => {
                if (part.startsWith('**') && part.endsWith('**')) {
                  return (
                    <strong key={pIdx} className="font-bold text-cyan-400">
                      {part.slice(2, -2)}
                    </strong>
                  );
                }
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-200">
        ...
        <div className="flex items-center gap-2">
          {onViewReports && (
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-xs font-semibold text-gray-300 hover:border-cyan-500/40 hover:text-white transition-colors flex items-center gap-1.5 cursor-pointer shadow-xs"
            >
      ...
      {currentResult && !isProcessing && (
        <div className="space-y-6 pt-4 animate-in fade-in duration-300">
          ...
          <div className="p-6 sm:p-7 rounded-2xl bg-gray-900/90 border border-white/10 shadow-xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
              ...
              {/* Confidence Badge */}
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 rounded-full bg-emerald-950/60 border border-emerald-500/30 text-emerald-300 text-xs font-bold flex items-center gap-1.5">
            ...
            <div className="pt-3 border-t border-white/10 flex flex-wrap items-center justify-between gap-3 text-xs">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center gap-1.5 transition-all shadow-xs cursor-pointer disabled:opacity-60"
                >
                  {isDownloadingPdf ? (
                    <>
