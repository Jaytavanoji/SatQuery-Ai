      ...
      <div
        ...
        className={`p-8 sm:p-10 rounded-2xl border-2 border-dashed text-center cursor-pointer transition-all duration-200 select-none ${
          isDragOver
            ...
            : 'border-white/20 hover:border-cyan-500/50 bg-white/[0.03] hover:bg-white/[0.06]'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        <div className="max-w-md mx-auto space-y-3 pointer-events-none">
          <div className="w-14 h-14 rounded-2xl bg-cyan-950/40 text-cyan-400 flex items-center justify-center mx-auto border border-cyan-500/30 shadow-xs">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Supports <strong className="text-cyan-400">satellite rasters (.tif, .tiff) only</strong>. Standard JPEG or PNG formats are not accepted.
            </p>
          </div>
        </div>
      </div>
        ...
        {images.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {images.map((img, index) => (
              <div
                key={img.id}
                className="p-4 rounded-xl bg-gray-900/90 border border-white/10 hover:border-cyan-500/40 transition-all shadow-sm space-y-3 group text-white"
              >
                {/* Header: Label & Delete */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-cyan-950/50 text-cyan-300 border border-cyan-500/30">
                    ...
                    <span className="text-xs font-semibold text-gray-400">{img.format}</span>
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <div
                    className="w-16 h-16 rounded-lg relative overflow-hidden border border-white/15 flex-shrink-0"
                    style={{ background: img.previewVisual || 'linear-gradient(135deg, #1e293b, #334155)' }}
                  />

                  <div className="min-w-0 flex-1 space-y-1">
                    <div className="text-xs font-bold text-white truncate" title={img.name}>
                      {img.name}
                    </div>

                    <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 text-[11px] text-gray-400">
                      <div>
                        <span>Dimensions: </span>
                        <strong className="text-gray-200">{img.dimensions}</strong>
                      </div>
                      <div>
                        <span>Type: </span>
                        <strong className="text-gray-200">{img.modality}</strong>
                      </div>
                      <div>
                        <span>Date: </span>
                        <strong className="text-gray-200">{img.acquisitionDate}</strong>
                      </div>
                      <div>
                        <span>Size: </span>
                        <strong className="text-gray-200">{img.sizeMb} MB</strong>
                      </div>
                    </div>
                  </div>
                </div>
