...
interface VisualEvidenceSplitterProps {
  result: AnalysisResultData;
}

export const VisualEvidenceSplitter: React.FC<VisualEvidenceSplitterProps> = ({ result }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-3 rounded-xl bg-gray-900/90 border border-white/10 flex flex-wrap items-center justify-between gap-3 text-xs shadow-lg backdrop-blur-md">
        ...
        <div className="flex items-center gap-1.5 p-1 rounded-lg bg-black/40 border border-white/10">
          <button
            ...
            className={`px-3 py-1 rounded-md transition-all cursor-pointer ${
              viewMode === 'split'
                ? 'bg-blue-600/30 text-cyan-300 border border-cyan-500/40 font-bold shadow-xs'
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 p-1 rounded-lg bg-black/40 border border-white/10">
            ...
            <span className="px-1.5 text-[11px] font-bold text-gray-300 font-mono">
              {Math.round(zoom * 100)}%
            </span>
            <button
          ...
          <button
            ...
            className={`px-2.5 py-1.5 rounded-lg border flex items-center gap-1.5 transition-all cursor-pointer ${
              maskVisible
                ? 'bg-blue-950/60 border-cyan-500/40 text-cyan-300 font-bold'
                : 'bg-black/40 border-white/10 text-gray-500'
            }`}
          >
            {maskVisible ? <Eye className="w-3.5 h-3.5 text-cyan-400" /> : <EyeOff className="w-3.5 h-3.5" />}
        ...
        {viewMode === 'side-by-side' && (
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4 h-[420px] sm:h-[480px]">
            ...
            >
              ...
              <span className="relative z-10 px-2.5 py-1 rounded bg-blue-950/80 border border-cyan-500/40 text-xs text-cyan-300 font-bold self-start backdrop-blur-sm">
              ...
              <span className="relative z-10 text-[11px] text-cyan-400 font-semibold">
        ...
        {viewMode === 'mask-overlay' && (
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative z-10 p-2.5 rounded-xl bg-slate-950/90 border border-slate-700 text-xs text-slate-300">
                {evidence.changeMap?.legend || 'AI Detection Mask displayed over satellite imagery'}
              </div>
              <button
                type="button"
                onClick={() => {
                  const bboxStr = JSON.stringify((result as any).bounding_boxes || []);
                  navigator.clipboard.writeText(bboxStr);
                  alert('Copied Bounding Box Coordinates to clipboard!');
                }}
              ...
              <button
                type="button"
                onClick={() => {
                  const geojsonUrl = getFullArtifactUrl(result.urls?.geojson_url);
                  if (geojsonUrl && geojsonUrl !== '#') {
                    window.open(geojsonUrl, '_blank');
                  } else {
                    navigator.clipboard.writeText(JSON.stringify((result as any).geojson || {}));
                    alert('Copied GeoJSON structure to clipboard!');
                  }
                }}
      ...
      {evidence.stats && evidence.stats.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {evidence.stats.map((st, i) => (
            <div
              key={i}
              className="p-3 rounded-xl bg-gray-900/90 border border-white/10 text-xs shadow-md backdrop-blur-md"
            >
              <div className="text-[10px] uppercase text-gray-400 font-semibold font-mono">{st.label}</div>
...
export const EvidenceViewer = VisualEvidenceSplitter;
