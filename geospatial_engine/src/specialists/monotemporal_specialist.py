...
from geospatial_engine.src.ingestion.raster_loader import GeoTIFFData
...
class SingleImageSpecialist:
    ...
    def __init__(
        ...
        # Attempt pre-trained timm feature backbone initialization
        try:
            import timm
            timm_name = "convnextv2_base.fcmae_ft_in22k_in1k" if self.modality == "optical" else "convnextv2_base"
            timm_m = timm.create_model(timm_name, pretrained=True, in_chans=self.in_channels, num_classes=0)
            # Transfer pre-trained stem and block weights where shapes match
            state_dict = timm_m.state_dict()
    ...
    def infer(
        ...
        # Grounding with calibrated physical radiometric prior & adaptive Otsu thresholding
        from geospatial_engine.src.physics.spectral_calculators import compute_otsu_threshold

        if self.modality == "optical":
            if target_name in ["water", "open_water", "flooded_land"]:
                if has_nir and b_nir is not None:
                    ndwi = (b_green - b_nir) / (b_green + b_nir + 1e-6)
                    otsu_t = compute_otsu_threshold(ndwi, val_range=(-1.0, 1.0))
                    cutoff = max(-0.10, min(0.20, otsu_t))
                    spectral_prob = 1.0 / (1.0 + np.exp(-12.0 * (ndwi - cutoff)))
                else:
                    ndwi_gr = (b_green - b_red) / (b_green + b_red + 1e-6)
                    ndwi_br = (b_blue - b_red) / (b_blue + b_red + 1e-6)
                    ndwi_visual = np.maximum(ndwi_gr, ndwi_br)
                    otsu_t = compute_otsu_threshold(ndwi_visual, val_range=(-1.0, 1.0))
                    cutoff = max(-0.05, min(0.15, otsu_t))
                    spectral_prob = 1.0 / (1.0 + np.exp(np.clip(-14.0 * (ndwi_visual - cutoff), -50.0, 50.0)))
                ...
                if has_nir and b_nir is not None:
                    ...
                    otsu_t = compute_otsu_threshold(ndvi, val_range=(-1.0, 1.0))
                    cutoff = max(0.10, min(0.40, otsu_t))
                    spectral_prob = 1.0 / (1.0 + np.exp(np.clip(-12.0 * (ndvi - cutoff), -50.0, 50.0)))
                else:
                    vari = (b_green - b_red) / (b_green + b_red - b_blue + 1e-6)
                    otsu_t = compute_otsu_threshold(vari, val_range=(-1.0, 1.0))
                    cutoff = max(0.05, min(0.25, otsu_t))
                    spectral_prob = 1.0 / (1.0 + np.exp(np.clip(-14.0 * (vari - cutoff), -50.0, 50.0)))
                target_prob = 0.25 * neural_norm + 0.75 * spectral_prob
