...
from geospatial_engine.src.controller.intent_router import QueryRouter, detect_sensor_modality
...
from geospatial_engine.src.ingestion.raster_loader import GeoTIFFData, RasterDataLoader as GeoTIFFLoader
from geospatial_engine.src.ingestion.tensor_preprocessors import PreprocessorDispatcher, create_valid_data_mask
from geospatial_engine.src.physics.spectral_calculators import PhysicsVerifier, SpatialVerifier, verify_detection_physics
from geospatial_engine.src.physics.geospatial_metrics import calculate_ground_metrics
from geospatial_engine.src.physics.contour_vectorizer import mask_to_geojson
from geospatial_engine.src.specialists.bitemporal_change_detector import ChangeDetectionSpecialist
from geospatial_engine.src.specialists.multimodal_fusion_specialist import CrossModalSpecialist
from geospatial_engine.src.specialists.monotemporal_specialist import SingleImageSpecialist
from geospatial_engine.src.export.result_visualizer import ArtifactVisualizer
from geospatial_engine.src.export.dossier_exporter import ReportGenerator
...
class SatQueryEngine:
    ...
    def query_vlm(
        ...
        """
        Sends user query to InternVL3.5-1B-Instruct (LoRA: fine-tuned-satellite-vlm) wrapped in strict anti-hallucination system context:
        ...
        The InternVL VLM processor MUST receive this standard 8-bit PIL Image, NOT the raw numpy array.
        ...
        Args:
            ...
            fallback_text: Pre-synthesized technical summary if 1B weights are offline.
    ...
    def execute_query(
        ...
        spectral_curve: Dict[str, float] = {}
        if primary_calibrated.count >= 8:
            band_names_s2 = ["B01 (CoAstal)", "B02 (Blue)", "B03 (Green)", "B04 (Red)", "B05 (RE1)", "B06 (RE2)", "B07 (RE3)", "B08 (NIR)", "B8A (Narrow NIR)", "B09 (Vapor)", "B11 (SWIR1)", "B12 (SWIR2)"]
            for idx, bname in enumerate(band_names_s2[:primary_calibrated.count]):
                band_arr = primary_calibrated.get_band(idx + 1)
                mean_val = float(np.mean(band_arr[binary_mask])) if np.sum(binary_mask) > 0 else float(np.mean(band_arr))
                spectral_curve[bname] = float(round(mean_val, 4))
        ...
        statistics = {
            ...
            "spectral_reflectance": spectral_curve,
...
GeospatialAIEngine = SatQueryEngine
