...
def mask_to_geojson(
    ...
    contours, _ = cv2.findContours(uint8_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    for cnt in contours:
        ...
        # Sub-pixel contour polygon sharpening (Douglas-Peucker simplification)
        epsilon_dp = 0.001 * cv2.arcLength(cnt, True)
        approx_cnt = cv2.approxPolyDP(cnt, epsilon_dp, True)

        pts = approx_cnt.squeeze(axis=1) if len(approx_cnt) >= 3 else cnt.squeeze(axis=1)
