...
from geospatial_engine.src.controller.data_schemas import BoundingBox, PhysicsVerificationResult
from geospatial_engine.src.ingestion.raster_loader import GeoTIFFData


def compute_otsu_threshold(arr: np.ndarray, num_bins: int = 256, val_range: Tuple[float, float] = (-1.0, 1.0)) -> float:
    """
    Adaptive Otsu Bimodal Thresholding: Dynamically calculates optimal threshold separating
    target spectral features (water, vegetation, urban) from background per satellite scene.
    Maximizes between-class variance: sigma_b^2(t) = w0(t) * w1(t) * [mu0(t) - mu1(t)]^2
    """
    flat = arr.flatten()
    ...
    counts, bin_edges = np.histogram(flat, bins=num_bins, range=val_range)
    ...
    mean1 = np.cumsum(counts * bin_centers) / (np.cumsum(counts) + 1e-7)
    mean2 = (np.cumsum((counts * bin_centers)[::-1])[::-1]) / (np.cumsum(counts[::-1])[::-1] + 1e-7)

    between_var = weight1 * weight2 * (mean1 - mean2) ** 2
    max_idx = np.argmax(between_var)
    optimal_thresh = float(bin_centers[max_idx])
